...
`SaleService` depends on sale/product/stock/movement/discount repositories, centralized `DocumentNumberService`, UnitOfWork and permission service. It owns pricing, numbering, discount and stock-decrement flow. `PurchaseService` creates purchases plus stock/movement; `PurchasePostingService` uses the same numbering service for journal entries while retaining the supplier invoice as source reference. Purchase and sale aggregates reject duplicate product lines. `ProductStockService` creates opening balances and direct target-quantity adjustments. `StockTransactionService` uses UnitOfWork and permits only manual AdjustmentIn/AdjustmentOut with a required reason. `StockTransferService` owns workflow, transactional numbering and transfer posting/cancellation. UnitOfWork uses Serializable database transactions and converts EF concurrency conflicts to a retryable data-change message.
